...
def images(request):
    ...
    p = Paginator(all_images,30)
    page_obj = p.page(current_page)
    print(page_obj.object_list)
    for image_name in page_obj.object_list:
        if not os.path.isfile("autolith/static/data/{}/{}/{}".format(dataset, folder, image_name)):
    ...
    return render(request, 'demo/images.html', {'images': page_obj.object_list, 'dataset': dataset,
                    'folder': folder, 'page_obj': page_obj} )
...
def unet_setup(request):
    ...
    if 'base' in initial:
        ...
        form = RunFormPhase1(initial=initial)
        return render(request, 'demo/ai_run.html', {'form': form, 'existing_results': existing_results})
...
def dataview_images(request, dataset, folder):
    ...
    if request.method == 'POST':
        ...
        if 'alpha' in request.POST:
            ...
            user_uploaded = False
            if dataset == 'datasets_user' and folder.startswith('raw'):
                user_uploaded = True
            return render(request, 'demo/dataview_images_list.html', 
        ...
        if 'mode' in request.GET:
            ...
            return render(request, 'demo/dataview_images_train.html', 
            ...
            if current_task == 'test':
                template_to_load = 'demo/dataview_images_test.html'
            else:
                template_to_load = 'demo/dataview_images_annotate.html'
            ...
            if error_found:
                ...
                return render(request, 'demo/dataview_images_ensemble_train.html', 
                    {'dataset': dataset, 'folder': folder, 'images': page_obj.object_list, 'all_models': all_models, 
            ...
            if error_found:
                ...
                return render(request, 'demo/dataview_images_ensemble_test.html', 
                    {'dataset': dataset, 'folder': folder, 'images': page_obj.object_list, 'all_models': all_models, 
            ...
            if current_task == 'list':
                ...
                return render(request, 'demo/dataview_images_annotate.html', 
                ...
                if disabled_edit:
                    all_h5 = glob.glob('datasets_user/{}_{}*/*.h5'.format(current_ai_method, current_model) )
                else:
                    all_h5 = glob.glob('datasets_*/{}_{}*/*.h5'.format(current_ai_method, current_model) )
                print(len(all_h5))
                all_models = []
                ...
                return render(request, 'demo/dataview_images_test.html', 
                    {'dataset': dataset, 'folder': folder, 'images': page_obj.object_list, 'all_models': all_models, 
...
def data_detail(request, dataset, folder, image_name):
    ...
    return render(request, 'demo/image_detail.html', {'dataset': dataset, 'folder': folder, 'image_name': image_name, 
        'current_process': current_process, 'sample_model_unet': sample_model_unet, 'sample_model_mrcnn': sample_model_mrcnn})



def dataview(request):
    return render(request, 'demo/dataview.html')
...
def upload(request):
    ...
    if request.method == 'POST':
        ...
        for ff in request.FILES.getlist('files'):
            ...
            if ff.name.endswith('.csv'):
                try:
                    os.makedirs("datasets_user/raw_{}_{}/0/".format(request.POST['folder'], data_id))
                except:
                    pass
                with open("datasets_user/raw_{}_{}/{}".format(request.POST['folder'], data_id, ff.name), "wb+") as dest:
                    for chunk in ff.chunks():
                        dest.write(chunk)
                with open("datasets_user/raw_{}_{}/{}".format(request.POST['folder'], data_id, ff.name)) as fin:
                    lines = fin.readlines()

                all_data_map = {}
                try:
                    with open("datasets_user/all_data_map.json") as jsonfin:
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        pass
                with open("datasets_user/all_data_map.json", "w") as jsonfout:
        ...
        for ff in request.FILES.getlist('outer_contour'):
            print(ff.name)
            with open("datasets_user/raw_{}_{}/0/{}".format(request.POST['folder'], data_id, ff.name), "wb+") as dest:
                for chunk in ff.chunks():
                    dest.write(chunk) 
        ...
        return redirect('/demo/dataview/datasets_user/?folder_type=raw')
    else:
    ...
    raw_data = True
    all_dir_paths = glob.glob('datasets_user/raw_*')
    print(all_dir_paths)
    all_folders = []
    count = 0
    for dir_path in all_dir_paths:
    ...
    return render(request, 'demo/upload.html', {'form': form, 'upload_type': upload_type, 'folders': all_folders })
...
def load_annotations(request, dataset, folder, drawing_tool='default'):
    ...
    if drawing_tool == 'default':
        return render(request, 'demo/researchers.html', {'pass_var': json_var, 'fname': 'outer', 'mode': 'outer', 'drawing_tool': 'brush', 'dataformat': 'outer', 'dataset': dataset, 'folder': folder})
    return render(request, 'demo/researchers.html', {'pass_var': json_var, 'fname': 'outer', 'mode': 'outer', 'drawing_tool': 'default', 'dataformat': 'outer', 'dataset': dataset, 'folder': folder})


def ensemble_setup(request):
    if request.method == 'POST':
        print(request.POST)
        raise ValueError
...
def show_data(request):
    ...
    #import wget
    #wget.download('https://zenodo.org/records/8341149/files/datasets_baltic.zip', out='datasets_baltic/images/')

    folders = []
    all_txt = glob.glob('datasets_north/*/*of*.txt')
    print(len(all_txt))
    for item in all_txt:
        strs = item.replace("\\", "/").split("/")
        folders.append([strs[-1], strs[-2], strs[-3], True, True])

    return render(request, 'demo/show_data.html', {'folders': folders})
...
def show_runs(request):
    ...
    if request.method == 'POST':
        dataset = request.POST['dataset_name']
        folder = request.POST['split_name']

        txt_files = glob.glob('{}/{}/*of*.txt'.format(dataset, folder))
        if len(txt_files)>0:
            fname = txt_files[0].replace("\\", "/").split("/")[-1]
        else:
    ...
    all_h5 = glob.glob('datasets_baltic/*/*.h5')
    print(len(all_h5))
    ...
    return render(request, 'demo/show_runs.html', {'folders': folders})
...
def experiments_textbased(request):
    return render(request, 'demo/experiments_textbased.html')

def experiments_transfer_learning(request):
    return render(request, 'demo/experiments_transfer.html')

def experiments_ensemble_learning(request):
    return render(request, 'demo/experiments_ensemble.html')

def experiments_continual_learning(request):
    return render(request, 'demo/experiments_continual.html')
