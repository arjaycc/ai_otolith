...
def train(name='mrcnn', data_params={}, edge_params={}, train_params={}, settings={}):
    ...
    with tf.device('/gpu:0'):
        ...
        for imgfile in ff:
            ...
            # if 'input_run1' in settings:
            #outer_bounds = load_outer_bound(settings['input_run1'])
            ...
            with open("{}/{}/output/bbox_{}.json".format(domain, settings['input_run1'], image_name)) as fin:
                ...
                #sep_name = fname.replace('_id{}_'.format('-'.join(ids)), '_id{}_'.format(id_info))
                #imgraw = imgraw[]
            ...
            #sqmaskraw = test_ds.resize_mask(maskraw, scale, padding)
            ...
            ai_reading, distances = measure_prediction_output(results, int(cx), int(cy), -1 )
            if ai_reading in age_distances:
                age_distances[ai_reading].append(distances)
            else:
                age_distances[ai_reading] = [distances]

        output_file = "{}/{}/distances.json".format(domain, name )
        with open(output_file, 'w') as fout:
            json.dump(age_distances, fout)
